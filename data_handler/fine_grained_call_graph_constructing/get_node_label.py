                ...
                with open(json_path, 'r') as f:
                    ...
                    for edge in data['edges']:
                        ...
                        if '_' in source_node:
                            ...
                            if len(source_parts) > 2:
                                source_node = '_'.join(source_parts[:2])
                        if source_node in normal:
                            cg_source.add(source_node)
                        if target_node in normal:
                            cg_target.add(target_node)
                    has_no_child = cg_target - cg_source
                    relay = relay - has_no_child

normal = normal - relay
print("Normal nodes:",len(normal),sorted(normal))
print("Relay nodes:",len(relay),sorted(relay))
...
with open("./sample_data/node_label.csv", "w", newline="") as csv_file:
    ...
    for label, elements in [("database", database), ("Memcached", Memcached), ("blackhole", blackhole), ("normal", normal), ("relay", relay)]:
